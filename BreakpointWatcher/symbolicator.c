	...
	struct mach_o_handler *handler, struct segments_handler *segments_info)
{
	...
	if (segments_info->symtab_ptr== NULL 
		|| segments_info->seg_linkedit_ptr.seg_linkedit_ptr_64 == NULL)
		return false;
	...
	struct mach_o_handler *handler, struct segments_handler *segments_info)
{
	...
	if (segments_info->symtab_ptr== NULL 
		|| segments_info->seg_linkedit_ptr.seg_linkedit_ptr_32 == NULL)
		return false;
...
static int64_t get_local_syms_in_vm32(mach_port_t target_task, const char *mysymbol,
	struct mach_o_handler *handler_ptr)
{
	...
	for (int64_t index = 0; index < handler_ptr->nsyms; index++, sym++) {
		...
		if (strcmp(handler_ptr->strings + cur_str_index, mysymbol) == 0) {
			printf("sym: %s \n", handler_ptr->strings + cur_str_index);
			return handler_ptr->vm_slide + dup_sym.n_value;
		}
...
static int64_t get_local_syms_in_vm64(mach_port_t target_task, const char* mysymbol,
	struct mach_o_handler *handler_ptr)
{
...
void *my_dyld(pid_t pid, const char *path, const char* mysymbol, struct mach_o_handler *handler_ptr)
{
	...
	uint64_t addr = get_lib_load_addr(target_task, path, &(handler_ptr->is_32bit));
	if (addr == 0) {
		mach_port_deallocate(mach_task_self(), target_task);
		return 0;
	}
	
	bool ret = prepare_symbolication(target_task, handler_ptr, (void *)addr, LOAD_ADDR);
	if (ret == 0) {
		mach_port_deallocate(mach_task_self(), target_task);
		return 0;
	}
	...
	if (handler_ptr->is_32bit) {
		return (void *)get_local_syms_in_vm32(target_task, mysymbol, handler_ptr);
	} else {
		return (void *)get_local_syms_in_vm64(target_task, mysymbol, handler_ptr);
	}
	return (void *)0;
}
