...
Divider::Divider(list<event_t *> & _ev_list, list<event_t *> & _bt_list, uint32_t _divide_type, event_t * _divide_event)
:ev_list(_ev_list)
{
...
callout_ev_t* Divider::get_divider(list<event_t *> &bt_list)
{
	if (bt_list.size() == 0)
		return NULL;
	...
	if (mach_msg_event != NULL) {
		...
		while (it != ev_list.end() && it != ev_list.begin()) {
			...
			if ((*it)->get_tid() == mach_msg_event->get_tid() 
				&& dynamic_cast<callout_ev_t*>(*it)) {
				cerr << "return divider is not null" << endl;
				return dynamic_cast<callout_ev_t*>(*it);
			}
		}
	}
	return NULL;
...
bool Divider::match_callevents(callcreate_ev_t* event,  callout_ev_t * divider)
{
...
bool Divider::match_callevents(callout_ev_t* event,  callout_ev_t * divider)
{
	if (event->get_func_ptr() == divider->get_func_ptr()
		&& event->get_param0() == divider->get_param0())
		return true;
...
bool Divider::match_callevents(callcancel_ev_t* event,  callout_ev_t * divider)
{
	if (event->get_func_ptr() == divider->get_func_ptr()
		&& event->get_param0() == divider->get_param0())
		return true;
...
void Divider::divide(void)
{
	...
	callout_ev_t * divider = dynamic_cast<callout_ev_t *>(divide_event);
	callcreate_ev_t * call_create;
	callout_ev_t * call_out;
	callcancel_ev_t * call_cancel;
	
	cerr << "try to divide" << endl;
	for (it = ev_list.begin(); it != ev_list.end(); it++) {
		call_create = dynamic_cast<callcreate_ev_t *>(*it);
		if (call_create) {
			if (match_callevents(call_create, divider))	{
				if (tmp_sub.size() > 0) {
					//sublists[counter++] = tmp_sub;
					cerr << "should not collect before callcreate " << fixed << setprecision(1) << (tmp_sub.front())->get_abstime() << endl;
					tmp_sub.clear();
				}
		...
		call_out = dynamic_cast<callout_ev_t *>(*it);
		if (call_out) {
		...
		call_cancel = dynamic_cast<callcancel_ev_t *>(*it);
		if (call_cancel) {
