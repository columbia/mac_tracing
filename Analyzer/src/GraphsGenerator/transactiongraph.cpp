...
Node *TransactionGraph::augment_node(Group *cur_group, EventBase *event_after)
{
	...
	node = new Node(this, cur_group);
    assert(check_and_add_node(node) == nullptr);

    //add new main thread node to map
    if (node->is_node_from_thread(get_main_tid())) {
        main_nodes[node->get_gid()] = node;
        end_node = node->contains_view_update() ? node : end_node;
    }

    //edge added by old_node are not added to outgoing again.
	std::map<Group *, Edge *> outgoing = get_outgoing_connections(node, event_after);
    std::map<Group *, Edge *>::iterator it;

    std::cout << "Augment Node " << std::hex << node->get_gid();    
    std::cout << " with #" << outgoing.size() << " nodes" << std::endl;

    for (it = outgoing.begin(); it != outgoing.end(); ++it) {
        std::cout << "\tadd edge from 0x" << std::hex << node->get_gid();
        std::cout << " to 0x" << it->second->e_to->get_group_id() << std::endl;
        Node *to_node = augment_node(it->first, (it->second)->e_to);
        if (to_node != nullptr) {
            if (it->second->is_weak_edge()) 
                to_node->add_in_weak_edge(it->second);
            else
                to_node->add_in_edge(it->second);
        }
        assert(to_node);
        assert(it->second->to);
...
void TransactionGraph::init_graph()
{
    ...
    std::cout << "Begin augment graph from input node id 0x" << std::hex << cur_gid << " ..." << std::endl;
	for(Group *next = groups_ptr->get_group_by_gid(cur_gid); next != nullptr;
		++cur_gid, next = groups_ptr->get_group_by_gid(cur_gid)) {

        if (end_node) {
            std::cout << "0x" << std::hex << end_node->get_gid() << " updates views in the graph" << std::endl;
            break;
        }

        if (begin == true && next->contains_nsappevent()) {
            std::cout << "0x" << std::hex << cur_gid << " has another AppKit event" << std::endl;
            break;
        }

        if (main_nodes.find(cur_gid) != main_nodes.end()) {
            std::cout << "0x" << std::hex << cur_gid << " has been included in the graph" << std::endl;
            continue;
        }
    
        std::cout << "Augment main thread node 0x" << std::hex << cur_gid << " ..." << std::endl;
    ...
    std::cout << "Finish augment graph";
    std::cout << " from 0x" << std::hex << root_node->get_gid();
    if (end_node != nullptr)
        std::cout << " to 0x" << std::hex << end_node->get_gid();
    std::cout << std::endl; 
    graph_statistics();
}
