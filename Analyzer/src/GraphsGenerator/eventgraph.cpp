...
{
    std::vector<CASetEvent*> ca_set_events = ca_display_event->get_ca_set_events();
    std::vector<CASetEvent*>::iterator it;
    for (it = ca_set_events.begin(); it != ca_set_events.end(); it++) {
        CASetEvent *ca_set_event = *it;
        Edge *e = new Edge(ca_set_event, ca_display_event, CA_REL);
        if (!node->add_in_edge(e))
            delete e;
    }
...
/*
void EventGraph::add_edge_for_wait(Node *node, WaitEvent *wait_event)
{
    MakeRunEvent *mkrun_event = wait_event->get_mkrun();
    if (mkrun_event && mkrun_event->get_group_id() != (uint64_t)-1) {
...
Edge *EventGraph::add_weak_edge(Node *node)
{
    ...
    assert(node->add_out_weak_edge(e));
	
    Node *to = nodes_map[next_group];
	assert(to);
    assert(!to->add_in_weak_edge(e));
    return e;
}
...
void EventGraph::init_graph()
{
    ...
    time(&time_begin);
    std::cerr << "Begin generate graph..." << std::endl;
    ...
    std::map<uint64_t, Group *> groups = groups_ptr->get_groups();
    ...
    time(&time_check);
    std::cerr << "graph generation costs "  << std::fixed << std::setprecision(1) << difftime(time_check, time_begin) << " seconds" << std::endl;
    graph_statistics();
}
