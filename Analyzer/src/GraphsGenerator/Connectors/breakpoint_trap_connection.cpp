    ...
    if (prev_val == 0 && val == 1) { //store writes
        ...
        if (var == "gOutMsgPending") {
            ...
            if (cur_event->check_read())
                ...
                std::cerr << "Y";
            std::cerr << "\t val = " << cur_event->get_trigger_val() << std::endl;
        }
        mtx.unlock();
#endif
...
#if DEBUG_BREAKPOINT_TRAP
            mtx.lock();
            if (prev_writes[var].size() > 1) {
                std::cerr << "More writes than reads on " << var << " before "\
                 << std::fixed << std::setprecision(1) << setcur_event->get_abstime() << std::endl'
            }
            mtx.unlock();
            while (prev_writes[var].size() > 1) {
...
#if DEBUG_BREAKPOINT_TRAP
        mtx.lock();
        std::cerr << "No Flip expected at current event at " << std::fixed << std::setprecision(1) << cur_event->get_abstime() << std::endl; 
        mtx.unlock();
#endif
    }
}
...
void BreakpointTrapConnection::breakpoint_trap_connection(void)
{
    ...
    mtx.lock();
    std::cerr << "begin breakpoint trap (shared variable) connection... " << std::endl;
    mtx.unlock();
#endif
...
#ifdef DEBUG_BREAKPOINT_TRAP
    mtx.lock();
    std::cerr << "finish breakpoint trap connection." << std::endl;
    mtx.unlock();
#endif
}
