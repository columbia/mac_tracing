...
void WaitEvent::streamout_event(std::ofstream &outfile)
{
    ...
    if (syscall_event) {
        outfile << "\twait from " << std::fixed << std::setprecision(1) << syscall_event->get_abstime();

//    if (syscall_event->get_ret_time() > 0)
//    outfile << "\tsyscall cost " << std::fixed << std::setprecision(1) << syscall_event->get_ret_time() - syscall_event->get_abstime() << std::endl;
    }
    outfile << "\ttime cost " << std::fixed << std::setprecision(1) << get_time_cost() << std::endl;
    outfile << std::endl;
}
