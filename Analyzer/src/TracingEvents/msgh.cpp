...
bool MsgHeader::set_msgh_id(uint64_t _msgh_id, msg_ev_t * prev_msg)
{
...
void MsgHeader::decode_remote_port(ofstream &outfile)
{
	if (remote_port) {
		mach_msg_bits_t rbit = MACH_MSGH_BITS_REMOTE(msgh_bits);
		const char * port_name = ipc_type_name64(rbit);
		outfile << "remote = " << hex << rport_name;
		outfile << "(0x" << hex << remote_port << ")";
		if (port_name)
			outfile << " (" << port_name << ")\n";
		else 
			outfile << " (type 0x" << hex << rbit << ")\n";
	} else {
		outfile << "remote = null\n";
	}
}

void MsgHeader::decode_local_port(ofstream &outfile)
{
	if (local_port) {
		mach_msg_bits_t lbit = MACH_MSGH_BITS_LOCAL(msgh_bits);
		const char * port_name = ipc_type_name64(lbit);
		outfile << "\t      \t\tlocal = " << hex << lport_name;
		outfile << "(0x" << hex << local_port << ")";
		if (port_name)
			outfile << " (" << port_name << ")\n";
		else
			outfile << " (type 0x" << hex << lbit << ")\n";
	} else {
		outfile << "\t      \t\tlocal = null\n";
	}
}

void MsgHeader::decode_voucher_port(ofstream &outfile)
{
	if (carried_voucher_port) {
		outfile << "\t      \t\tvoucher = " << hex << carried_voucher_port << endl;
	} else {
		outfile << "\t      \t\tvoucher = null" << endl;
	}
}

void MsgHeader::decode_header(ofstream &outfile)
{
	string recv_send = recv ? "recv" : "send";
	outfile << "\t" << recv_send << "\t\n";
	outfile <<"\tPORTS:\t\t";
	...
	if (msgh_bits & ~MACH_MSGH_BITS_USED) {
		if (need_comma)
			outfile << ", unused = 0x" << hex << (msgh_bits & ~MACH_MSGH_BITS_USED) << endl;
		else
			outfile << "\tunused = 0x" << hex << (msgh_bits & ~MACH_MSGH_BITS_USED) << endl;
	}
	if (from_kernel == true)
		outfile << "send from kernel" << endl;
	if (mig == true) 
		outfile << "\n\tmig[" << dec << msgh_id << "]: " <<::LoadData::mig_dictionary[msgh_id].c_str() << endl;
	outfile << endl;
}
