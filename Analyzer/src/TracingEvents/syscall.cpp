...
bool SyscallEvent::audit_args_num(int size)
{
    if (!sc_entry) {
        ...
        mtx.lock();
        std::cerr << "sc is null at " << std::fixed << std::setprecision(1) << get_abstime() << std::endl;
        mtx.unlock();
#endif
        return false;
    }
...
#if DEBUG_SYSCALL_EVT
        mtx.lock();
        std::cerr << "args number " << nargs + size << " exceeds " << MAX_ARGC\
            << " at " << std::fixed << std::setprecision(1) << get_abstime() << std::endl;
        mtx.unlock();
#endif
        return false;
    }
...
#if DEBUG_SYSCALL_EVT
        mtx.lock();
        std::cerr << "args index " << nargs << " is invalid at "\
            << std::fixed << std::setprecision(1) << get_abstime() << std::endl;
        std::cerr << "Prev syscall doesn't returned. Most possible a new syscall event begins." << std::endl;
        mtx.unlock();
#endif
        return false;
    }
