...
#if DEBUG_VOUCHER
        mtx.lock();
        std::cerr << "No comm found for pid " << std::hex << pid << std::endl;
        mtx.unlock();
#endif
        return proc_comm;
    }
    ...
    void init_lib_info(std::string lib_info_file)
    {
        ...
        if (access(meta_data.symbolicate_procs_file.c_str(), R_OK) == 0) {
            ...
            while(getline(input, proc_name)) {
                std::cerr << "load lib for " << proc_name << std::endl;
        ...
        if (symbolic_procs.size() > 0) {
            ...
            for (it = symbolic_procs.begin(); it != symbolic_procs.end(); it++) {
                ...
                if (!load_lib(proc_name, lib_info_file)) {
                    load_success = false;
                    std::cerr << "Fail to load [user input process] lib for " << proc_name << std::endl;
                }
            }
        }
        visit_map.clear();
