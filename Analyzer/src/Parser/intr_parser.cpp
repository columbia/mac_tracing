...
namespace Parse
{
	...
	// symbolize rip
	void IntrParser::symbolize_rips(BacktraceParser *backtrace_parser)
	{
		...
		for (it = local_event_list.begin(); it != local_event_list.end(); it++) {
			...
			if (ret == true) {
				string rip_symbol = frame_info.symbol + "_at_" + frame_info.filepath;
	...
	intr_ev_t * IntrParser::create_intr_event(double abstime, string opname, istringstream & iss) 
	{
		string procname;
		...
		intr_ev_t * new_intr = new intr_ev_t(abstime, opname, tid, intr_no, rip, user_mode, coreid, procname);
	...
	{
		string procname;
		uint64_t intr_no, ast, effective_policy, request_policy, tid, coreid;
		if (!(iss >> hex >> intr_no >> ast >> effective_policy >> request_policy)
			|| !(iss >> hex >> tid >> coreid))
			return false;
		...
		intr_event->set_request_policy(request_policy);
	...
	void IntrParser::process()
	{
		...
		while (getline(infile, line)) {
			...
			if (is_begin) {
				intr_ev_t * new_intr = create_intr_event(abstime, opname, iss);
				if (!new_intr)
					outfile  << line << endl;
			} else {
				bool ret = collect_intr_info(abstime, iss);
				if (!ret)
					outfile << line << endl;
			}
		}
