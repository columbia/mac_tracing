...
namespace Parse
{
	...
	bool WaitParser::set_info(wait_ev_t *wait_event, uint64_t pid,
	...
	bool WaitParser::new_wait_event(string opname, double abstime,
		istringstream &iss)
	{
		uint64_t disc[3] = {0};
		uint64_t wait_event, tid, coreid;
		string procname;
		if (!(iss >> hex >> wait_event >> disc[0] >> disc[1] >> disc[2] \
			>> tid >> coreid))
			return false;
		...
		if (wait_events.find(tid) != wait_events.end()) {
			outfile << "Second wait event for the same tid at " << fixed << setprecision(1) << abstime << endl;
			return false;
		}
		//assert(wait_events.find(tid) == wait_events.end());
		wait_ev_t *new_evw = new wait_ev_t(abstime, "MACH_wait", tid, wait_event,
	...
	bool WaitParser::finish_wait_event(string opname, double abstime,
		istringstream &iss)
	{
		uint64_t wait_event, pid, deadline, wait_result, tid, coreid;
		if (!(iss >> hex >> wait_event >> pid >> deadline >> wait_result \
			>> tid >> coreid))
			return false;
		...
		if (wait_events.find(tid) == wait_events.end()) {
			...
			wait_ev_t *wait_evt = new wait_ev_t(abstime, opname, tid,
