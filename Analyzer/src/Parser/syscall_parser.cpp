	...
	std::string opname, uint64_t *args, int size)
{
    ...
    if (syscall_event->set_args(args, size) == false) {
        mtx.lock();
        std::cerr << "Fail to set args for syscall at "\
			<< std::fixed << std::setprecision(1) << syscall_event->get_abstime() << std::endl;
        assert(false);
        mtx.unlock();
    }
    ...
    if (syscall_events.find(tid) != syscall_events.end()) {
        ...
        if ((syscall_event->get_op() != opname) || (!is_end && !syscall_event->audit_args_num(1))) {
            ...
            if (is_end) {
                ...
                mtx.lock();
                std::cerr << "Error: Fail to set args for syscall at "\
					<< std::fixed << std::setprecision(1) << abstime << std::endl;
                assert(false);
                mtx.unlock();
                exit(EXIT_FAILURE);
            } 
