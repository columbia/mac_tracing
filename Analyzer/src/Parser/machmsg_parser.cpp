...
namespace Parse
{
	...
	{
		msg_events.clear();
		collector.insert(pair<string, uint64_t>("MACH_IPC_kmsg_free",
			 MACH_IPC_kmsg_free));
		collector.insert(pair<string, uint64_t>("MACH_IPC_msg_trap",
			MACH_IPC_msg_trap));
		collector.insert(pair<string, uint64_t>("MACH_IPC_msg_link",
			MACH_IPC_msg_link));
		collector.insert(pair<string, uint64_t>("MACH_IPC_msg_send",
			MACH_IPC_msg_send));
		collector.insert(pair<string, uint64_t>("MACH_IPC_msg_recv",
			MACH_IPC_msg_recv));
		collector.insert(pair<string, uint64_t>("MACH_IPC_msg_recv_voucher_re",
			MACH_IPC_msg_recv));
		//collector.insert(pair<string, uint64_t>("MACH_IPC_msg_boarder",
		//	MACH_IPC_msg_boarder));
		//collector.insert(pair<string, uint64_t>("MACH_IPC_msg_thread_voucher",
		//	MACH_IPC_msg_thread_voucher));
		//collector.insert(pair<string, uint64_t>("MACH_IPC_msg_dump",
		//	MACH_IPC_msg_dump));
		//collector.insert(pair<string, uint64_t>("MACH_IPC_msg_dsc",
		//	MACH_IPC_msg_dsc));
		//collector.insert(pair<string, uint64_t>("MACH_IPC_msg_idsc",
		//	MACH_IPC_msg_idsc));
	}
	 ...
	 */
	msg_ev_t *MachmsgParser::check_mig_recv_by_send(msg_ev_t *cur_msg,
		uint64_t msgh_id)
	{
		...
		for (++rit; rit != local_event_list.rend(); rit++ ) {
			...
			/*
			outfile << "Checking msg " << fixed << setprecision(1) \
					<<  msg->get_abstime() << " for " \
					<< fixed << setprecision(1) << cur_msg->get_abstime() \
					<< endl;
			if (msg->is_freed_before_deliver())
				outfile << "is free before_delivery" << endl;
			outfile << "msg tid " << hex << msg->get_tid() \
					<< "\t cur_msg tid " << hex << cur_msg->get_tid() << endl;
			*/
	...
	bool MachmsgParser::process_kmsg_end(string opname, double abstime,
		istringstream &iss)
	{
		uint64_t kmsg_addr, user_addr, rcv_or_send, unused, tid, coreid;
		string procname;

		if (!(iss >> hex >> kmsg_addr >> user_addr >> rcv_or_send >> unused \
			>> tid >> coreid))
			return false;
	...
	bool MachmsgParser::process_kmsg_set_info(string opname, double abstime,
		istringstream &iss)
	{
		uint64_t kmsg_addr, portnames, remote_port, local_port, tid, coreid;
		string procname;

		if (!(iss >> hex >> kmsg_addr >> portnames >> remote_port \
	...
	msg_ev_t *MachmsgParser::new_msg(string msg_op_type, uint64_t tag,
		double abstime, uint64_t tid, uint64_t coreid, string procname)
	{
		...
		msg_ev_t *new_msg = new msg_ev_t(abstime, msg_op_type, tid, msgh_info,
	...
	bool MachmsgParser::process_kmsg_begin(string opname, double abstime,
		istringstream &iss)
	{
		...
		istringstream &iss)
	{
		...
		if (!(iss >> hex >> kmsg_addr >> unused >> unused >> unused \
			>> tid >> coreid))
			return false;
