...
namespace Parse
{
    MkrunParser::MkrunParser(std::string filename)
    :Parser(filename)
    {
        mkrun_events.clear();
    }

    bool MkrunParser::set_info(MakeRunEvent *mr_event, uint64_t peer_prio,
    ...
    bool MkrunParser::process_finish_wakeup(double abstime, std::istringstream &iss)
    {
        std::string procname;
    ...
    bool MkrunParser::process_new_wakeup(double abstime, std::istringstream &iss)
    {
        std::string procname;
