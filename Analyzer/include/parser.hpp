...
namespace Parse
{
	...
	public:
		...
		void process();
		void symbolize_rips(BacktraceParser *backtrace_parser);
	};
	...
	class DispatchParser: public Parser {
		...
		bool process_execute(string opname, double abstime, istringstream &iss);
	public:
		DispatchParser(string filename);
		void process();
		void symbolize_ctxt_and_func(BacktraceParser *backtrace_parser);
		void symbolize_invoke_func(BacktraceParser *backtrace_parser);
	};
	...
	public:
		...
		//TODO move clear to ~BacktraceParser();
		~BacktraceParser() {clear();}
		images_t * get_host_image();
		map<string, map<uint64_t, string> > &get_vmsymbol_map();
		bool setup_lldb(debug_data_t *degugger_data, images_t * images);
		void process();
		void clear();
	};
