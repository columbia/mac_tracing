...
class Groups {
	...
	map<uint64_t, gid_group_map_t> sub_results;
...
public:
	...
	uint64_t get_main_thread() {return main_thread;}
	map<uint64_t, group_t *> &get_groups(void) {return groups;}
	map<uint64_t, group_t *> &get_main_groups() {return main_groups;}
	map<uint64_t, group_t *> &get_host_groups() {return host_groups;}
	map<uint64_t, group_t *> &get_groups_by_tid(uint64_t tid) {return sub_results[tid];}
