...
public:
	...
	string get_sym_for_addr(uint64_t vm_offset, map<uint64_t, string> & vm_sym_map);
	void checking_symbol_with_image_in_memory(string &symbol, uint64_t vm, string path, map<string, map<uint64_t, string> >&);
