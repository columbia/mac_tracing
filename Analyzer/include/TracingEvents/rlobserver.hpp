...
class RLObserverEvent:public EventBase {
	uint64_t rl;
	uint64_t mode;
	rl_stage_t stage;
public:
	RLObserverEvent(double timestamp, string op, uint64_t tid, uint64_t _rl, uint64_t _mode, uint64_t _stage, uint32_t event_core, string procname = "");
