...
#include "base.hpp"
using namespace std;
class CADispEvent : public EventBase {
	uint64_t object_addr;
	uint64_t is_serial;
	vector<ca_set_ev_t *> object_set_events;
public:
	CADispEvent(double timestamp, string op, uint64_t tid, uint64_t object, uint32_t coreid, string procname = "");
	void push_set(ca_set_ev_t * set) {object_set_events.push_back(set);}
	...
	vector<ca_set_ev_t *>& get_ca_set_events(void) {return object_set_events;}
