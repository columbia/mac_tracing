...
public :
    ...
    void pair_mkrun(MakeRunEvent *mkrun) {set_event_peer(mkrun);} //{mkrun_event = mkrun; set_event_peer(mkrun);}
    void pair_syscall(SyscallEvent *syscall) {syscall_event = syscall;}
    MakeRunEvent *get_mkrun(void){return dynamic_cast<MakeRunEvent*>(get_event_peer());} //{return mkrun_event;}
