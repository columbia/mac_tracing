...
void Cluster::compare(cluster_t *peer, ofstream &output)
{
	vector<rel_t> peer_edges = peer->get_edges();
	vector<rel_t>::iterator edge_it1, edge_it2;
	vector<rel_t> miss_edges, peer_matched_edges;
	output << "Compare cluster " << hex << cluster_id << " and " << hex << peer->get_cluster_id() << endl;
	...
	vector<group_t *> peer_nodes = peer->get_nodes();
	vector<group_t *>::iterator node_it1, node_it2;
	vector<uint64_t> miss_gid, peer_matched_gid;
	...
	output << "< Nodes only in cluster " << hex << cluster_id << ":";
	int i = 0;
	for (vector<uint64_t>::iterator it = miss_gid.begin(); it != miss_gid.end(); it++) {
		if (i % 8 == 0)
			output << endl;
		output << hex << *it << "\t";
		i++;
	...
	output << "< Edges only in cluster " << hex << cluster_id << ":" << endl;
	for (edge_it1 = miss_edges.begin(); edge_it1 != miss_edges.end(); edge_it1++)
		decode_edge(output, *edge_it1);

	output << "> Nodes only in cluster " << hex << peer->get_cluster_id() << ":";
	i = 0;
	for (node_it2 = peer_nodes.begin(); node_it2 != peer_nodes.end(); node_it2++) {
		vector<uint64_t>::iterator it = find(peer_matched_gid.begin(), peer_matched_gid.end(),
				(*node_it2)->get_group_id());
		if (it == peer_matched_gid.end()) {
			if (i % 8 == 0)
				output <<  endl;
			output << hex << (*node_it2)->get_group_id() << "\t";
			i++;
		}
	}
	output << endl;

	output << "> Edges only in cluster " << hex << peer->get_cluster_id() << ":" << endl;
	for (edge_it2 = peer_edges.begin(); edge_it2 != peer_edges.end(); edge_it2++) {
		rel_t edge  = *edge_it2;
		vector<rel_t>::iterator edge_it = find_if(peer_matched_edges.begin(), peer_matched_edges.end(), 
				[&target_edge = edge]
				(const rel_t &m) -> bool{return target_edge == m;});
...
bool Cluster::traverse_connection(group_t *cur_node, group_t *destination_node, bool *visited,
		node_edges_map_t &to_edges, node_edges_map_t &from_edges, vector<rel_t> &paths)
{
	multimap_range_t from_range = from_edges.equal_range(cur_node);
	multimap_range_t to_range = to_edges.equal_range(cur_node);
	if (from_range.first == from_range.second && to_range.first == to_range.second) 
	...
	node_edges_map_t::iterator it;
	for (it = from_range.first; it != from_range.second; it++) {
		group_t *peer_node = (it->second).g_to;
#if DEBUG_CONN_TRAVERSE 
		cerr << "check connection: " << hex << cur_node->get_group_id() << " and " << hex << peer_node->get_group_id() << endl;
#endif
		if (visited[get_node_idx_in_cluster(peer_node)] == true)
			continue;
	...
	for (it = to_range.first; it != to_range.second; it++) {
		group_t *peer_node = (it->second).g_from;
#if DEBUG_CONN_TRAVERSE 
		cerr << "check connection: " << hex << cur_node->get_group_id() << " and " << hex << peer_node->get_group_id() << endl;
#endif
		if (visited[get_node_idx_in_cluster(peer_node)] == true)
...
void Cluster::check_connection(group_t *group_1, group_t *group_2, ofstream &outfile)
{
	node_edges_map_t to_edges, from_edges;
	classify_cluster_edges(to_edges, from_edges);
	...
	if (traverse_connection(group_1, group_2, visited, to_edges, from_edges, paths)) {
		...
		outfile << "Path from " << hex << group_1->get_group_id() << " to " << hex << group_2->get_group_id() << endl;
		vector<rel_t>::iterator edge_it;
		for (edge_it = paths.begin(); edge_it != paths.end(); edge_it ++) {
			//outfile << "\trel " << (*edge_it).e_from->get_procname();
			//outfile << " " << std::right << hex << (*edge_it).g_from->get_group_id();
		...
		outfile << "No connection get recognized !!!" << endl;
	}
}
...
void Cluster::sharevariable_edge(ofstream &outfile, event_t *event, bool *comma)
{
}

void Cluster::runloop_item_edge(ofstream &outfile, event_t *event, bool *comma)
{
}
...
void Cluster::decode_edge(ofstream &outfile, rel_t edge)
{
	if (edge.g_from == edge.g_to)
		return;

	uint32_t type = edge.rel_type;
	event_t *from_event = edge.e_from;
	event_t *to_event = edge.e_to;

	outfile << hex << edge.g_from->get_group_id();
	outfile << "\t" << from_event->get_procname() << ", ";
	outfile << hex << edge.g_to->get_group_id();
