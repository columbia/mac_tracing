...
{
	groups_t *groups_ptr = clusters_ptr->get_groups_ptr();
	map<uint64_t, group_t *> &host_groups = groups_ptr->get_host_groups();
...
int Filter::back_traverse_from(group_t *colored_group, bool *visited, cluster_t *cur_cluster,
...
cluster_t *Filter::filter_by_traverse(int index, cluster_t *cur_cluster)
{
	...
	vector<group_t*> groups = cur_cluster->get_nodes();
	...
	group_t *colored_group;
	for (it = groups.begin(); it != groups.end(); it++) {
		if (host_groups.find((*it)->get_group_id()) == host_groups.end()
			|| dst_cluster->get_node_idx_in_cluster(*it) != -1)
			continue;
		colored_group = *it;
		//cerr << "Add node " << hex << colored_group->get_group_id() << " from Cluster " << hex << cur_cluster->get_cluster_id() << endl;
...
bool Filter::read_processes(string filename)
{
	...
	if (input.fail()) {
		cerr << "No file found for process intersection" << endl;
		return false;
	}
...
bool Filter::audit_deletion_of_node(group_t *cur_node, node_edges_map_t &edges)
{
...
void Filter::clusters_filter_para(uint64_t type)
{
	...
	work.reset();
	threadpool.join_all();
	cerr << "Finished Filtering with type: ";
	if (type == PATH_FILTER)
		cerr << "PATH TRAVERSE" << endl;
	else if (type == PROCESS_FILTER)
		cerr << "PROCESS INTERSECTION" << endl;
}
...
void Filter::streamout_filtered_clusters(string output_path)
{
	...
	map<uint64_t, cluster_t *> &result_clusters = path_filter? filtered_clusters : original_clusters;
	map<uint64_t, cluster_t *>::iterator it;
