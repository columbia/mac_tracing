...
Log_trace()
{
	...
	memset(proclogfile, 0, 80);
	memcpy(proclogfile, logfile, strlen(logfile) > 64 ? 64 : strlen(logfile));
	strcat(proclogfile, "_procs_pre");
	store_proclist(proclogfile);
	...
	memset(proclogfile, 0, 80);
	memcpy(proclogfile, logfile, strlen(logfile) > 64 ? 64 : strlen(logfile));
	strcat(proclogfile, "_procs_post");
	store_proclist(proclogfile);
}
#endif
...
void read_trace()
{
	...
	if (!readRAW_flag) {
		...
		char proclogfile[80];
		memset(proclogfile, 0, 80);
		memcpy(proclogfile, RAW_file, strlen(RAW_file) > 64 ? 64 : strlen(RAW_file));
		strcat(proclogfile, "_procs_pre");
		load_proclist(proclogfile);
	...
	for (;;) {
		...
		for (kdp = &kd[0], i = 0; i < count; i++, kdp++) {
			...
			if ( !lkp) {
				...
				int t_thread;
#ifndef ORIGINAL
				if (debugid == MACH_WAKEUP_REASON) {
					save_tp_maps(kdp->arg1, kdp->arg4 >> 32);
					save_tp_maps(kdp->arg5, kdp->arg4);
				}
				
				if (debugid == MACH_WAIT_DEBUGID) {
					save_tp_maps(kdp->arg5, kdp->arg2);
				}
#endif
