...
We evaluated \xxx on \nbug real-world spinning-cursor issues in widely
used applications such as the Chromium browser and MacOS System
Preference, Installer application, and Notes.  The root causes of all
\nbug issues were, to the best of our knowledge, previously unknown to the
public. Our results show that \xxx is effective: we were able to use it to
find all root causes of issues, including the Chromium issue that remained
open for seven years.  \xxx is also fast: it systems-wide tracing incurs
only XX\% overhead.
