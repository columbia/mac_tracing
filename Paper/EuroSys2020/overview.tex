...
(\S\ref{subsec:eventgraph}). Then we explains how \xxx idenfity where the
vertex corresponding to the performance anomaly exists in the event graphs
(\S\ref{subsec:indicator}). In the preceeding section(\S\ref{subsec:overflow}),
we describe how \xxx diagnozes performance issue interactively with a concrete
example.
