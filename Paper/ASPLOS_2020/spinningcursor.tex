...
variables, ``\vv{is\_mainthread\_spinning}'' and
``\vv{dispatch\_to\_mainthread}'', are critical in the desgin. The NSEvent
...
spinning with ``\vv{is\_mainthread\_spinning}'' equals 0 ,
``\vv{dispatch\_to\_mainthread}'' will be set to 1 and a timer is armed. If the
...
``\vv{CGSConnectionSetSpinning}'' to WindowServer from the timer handler, and
