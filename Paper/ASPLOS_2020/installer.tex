When \vv{Installer} pops up a window for privileged permission during the
installation of \vv{jdk-7u80-macosx-x64}, moving the cursor out of the popup
...
daemon \vv{authd} blocks on semaphore. The root cause is the authentication
of user's previledge synchronously in the main thread, instead of the handler
for moving cursor. The result was verified with \vv{lldb}. The main thread
blocks in a synchronous message to \vv{authd} in API \vv{[IFRunnerProxy\
requestKeyForRights:askUser:]}.
