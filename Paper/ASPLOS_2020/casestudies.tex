...
for the \vv{NSApp event}. If the main thread fails to process it before the timer
...
window. Moreover, while exploiting the way \vv{NSEvent thread} communicates with
main thread, we found two variables, ``\vv{is\_mainthread\_spinning}'' and
