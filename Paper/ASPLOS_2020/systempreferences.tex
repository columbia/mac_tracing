...
\vv{DisableMonitor}~\cite{disablemonitor} completes its function with enable/disable
monitors online. We blocked on the spinning cursor while disabling an external
monitor and rearranging windows in \vv{Display} panel.
...
\vv{mach\_msg} and \vv{thread\_switch}, which falls into the category of
...
``\vv{\_gCGDidReconfigureSeen}'' is missing in the spining case, where the main
