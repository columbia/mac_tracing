...
peer process exists in \vv{IPC\_peer\_set}, and adds the message into the
...
\For{Event: Events in a thread}
	...
	\Case {Mach\_message}\label{graphcomputing:1.d}
		\If {peer $\in$ $IPC\_peer\_set$ or $IPC\_peer\_set$ $equals$ $\emptyset$}
...
snippet from the \vv{fontd} daemon, function \vv{dispatch\_execute} is
...
%%This data dependency needs to be captured in order to establish a
