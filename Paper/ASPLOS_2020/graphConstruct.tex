...
\For{Event: Connection Events}
	...
	\Case {Mach\_message}\label{graphcomputing:2.b}
		...
		\If {needs reply}
			\State {AddEdge(receiver, reply\_sender)}
		\EndIf
	\EndCase
...
daemon, function \vv{dispatch\_execute} is installed as a callback to a work
from dispatch queue. It subsequently calls \vv{dispatch\_mig\_server()} which
...
unrelated event from differnt processed in its event loop,
as shown in Figure~\ref{fig:batchingineventprocessing}. This batch processing
