...
Prior work proposed what we call \emph{Causal tracing}, a powerful technique to construct
...
Compared to debuggers such as \vv{Spindump} that capture only the current
system state, causal tracing is quite effective at aiding developers to
understand complex causal behaviors and pinpoint real-world performance
issues.
...
using one system call \vv{mach\_msg\_overwrite\_trap}, presumably to reduce
...
``\vv{need\_display} = 1'' in macOS animation rendering) and later queries
...
In this work, we present \xxx, an interactive system for debugging
