...
investigating a performance problem in Chromium. The system wide tracing tool
integrated in \xxx generates log file, which is the input of \xxx's event graph
construction component. The generated event graph is used by the interactive
debugger to slice causal path. It supports interactive search by providng
information for user to make decision in case of multiple predecessors in a
vertex. As shown in Figure~\ref{fig:argus-overview}, the debugger asks to choose
one edge within a subgraph. the output from \xxx is the vertexs related to the
root casue. In Chromium case, they are two vertices which form circularly wait.
