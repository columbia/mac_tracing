...
compared the baseline path with the spinning case, as shown in \vv{path 1}
...
the \vv{renderer} to return the string bounding box for a string, and
...
To verify this diagnosis, we shortened the
timeout in the \vv{pthread\_cond\_timedwait()} call in Chromium
to 150 milliseconds, which proportionally reduced how often this 
spinning cursor occurs.
