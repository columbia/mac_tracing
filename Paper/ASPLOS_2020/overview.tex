...
\State {$ControlFlowGraph$ $\gets$ ComputeGraph($HeuristicsSet$, $TracingEvents$)}\label{algmain:recompute}
\State {$SpinningNode$ $\gets$ search $ControlFlowGraph$ with timestamp
		when spinning cursor displays}\label{algmain:findspinningnode}
\If {$SpinningNode$ $==$ $CPU busy execution(livelock)$}
	\State{run backward traversal of edges until UI event found}\label{algmain:1st}
\Else
