...
\Function{Diagnose}{g, spinning\_node}
  \Switch {spinning\_node.block\_type}
	...
	\Case {LongWait}
		...
		\State {similar\_node $\gets$ node has similar event sequence to spinning\_node}
		%\State {baseline\_node $\gets$ ask user to pick from similar\_nodes} 
		\State {baseline\_path $\gets$ InteractiveSlice(similar\_node)} 	
		\For {each t in $\{$threads in baseline\_path$\}$}
			\State {$node_t$ $\gets$ node in t before spinning\_node gets spinning}
