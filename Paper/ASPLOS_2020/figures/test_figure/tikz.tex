...
\tikzstyle{gthread} = [label={[font=\scriptsize, red]below:#1}]
\tikzstyle{gnode} = [draw, thick, font=\scriptsize, align =left, minimum width=18pt, minimum height=36pt]
...
\node [apps, name = software1] at (0, 0) {chromium};% {\includegraphics[width=20pt]{./chromium_logo.png}};
...
\node (InteractiveDebugger) [systemComp, rounded corners=20pt, minimum height=250pt, minimum width=280pt, below left = 40pt and -160pt of log] {};
\node (debugger)[minimum width=220pt, above = -30pt of InteractiveDebugger]{Interactive Debugger};
\node (graph) [minimum height = 180pt, minimum width = 200pt, below = 10pt of debugger] {};
\node (shape) [doc, minimum height = 180pt, minimum width = 200pt, below = 10pt of debugger]{};
%\node [minimum width = 100pt, above = -20pt of graph, label=right:{graph}]{};
\node(t0s)[gthread = browser, minimum width = 2pt, below left = -180pt and -50pt of graph, font=\scriptsize]{tid0};
\node(t0e)[gthread = browser, minimum width = 2pt, below left = -20pt and -44pt of graph]{};
\node(t1s)[gthread = kern\_task, minimum width = 2pt, below left = -180pt and -85pt of graph, font=\scriptsize]{tid1};
\node(t1e)[gthread = kern\_task, minimum width = 2pt, below left = -20pt and -79pt of graph]{};
\node(t2s)[gthread = renderer, minimum width = 2pt, below left = -180pt and -120pt of graph, font=\scriptsize]{tid2};
\node(t2e)[gthread = renderer, minimum width = 2pt, below left = -20pt and -114pt of graph]{};
\node(t3s)[gthread = renderer, minimum width = 2pt, below left = -180pt and -155pt of graph, font=\scriptsize]{tid3};
\node(t3e)[gthread = renderer, minimum width = 2pt, below left = -20pt and -149pt of graph]{};
\node(t4s)[gthread = fontd, minimum width = 2pt, below left = -180pt and -185pt of graph, font=\scriptsize]{tid4};
...
\node(rdm2) [draw, thick, fill = blue!20, minimum height = 10pt, below = 18pt of rdm1, font=\scriptsize]{msg\\sema\_wait};
...
\draw[->] (brw0.east) -- node[font=\scriptsize, minimum width=5pt, pos=0.1, below]{e0} (rdw0.west);
\draw[->] (k0.east) -- node[font=\scriptsize, minimum width=5pt, above]{e1} (rdw0.west);
...
\node (1)[below left = -20pt and -50pt of legend] {};
\node (2)[right=8pt of 1]{};
\draw [-, solid] (1.east) -- (2);
\node [left = 10pt of 2, font=\small]{thread\_running};

\node (3)[below left = -30pt and -50pt of legend] {};
\node (4)[right=8pt of 3]{};
\draw [-, dotted] (3.east) -- (4);
\node [left =10pt of 3, font=\small]{thread\_wait};

\node (5)[below left = -40pt and -50pt of legend] {};
\node (6)[right =8pt of 5]{};
\draw [->] (5.east) -- (6);
\node [left =10pt of 5, font=\small]{edge};

\node [draw, solid, minimum width = 8pt, minimum height = 4pt, below left = -60pt and -50pt of legend, label={[font=\small]left:node}]{};



%draw interactive Debugging part
\node (user)[alice, minimum size=30pt, left = 100pt of InteractiveDebugger, node distance=100pt]{};
...
\node (result) [draw, dotted, minimum height = 100pt, minimum width = 400pt,pattern= north west lines, pattern color = black!30, below = 50pt of InteractiveDebugger]{};
