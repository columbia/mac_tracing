...
\draw [vecarrow, out=280, in=230] (user.south) + (0, -0.5) to node[below]{travers backward from e0} (query4.north);
...
\node (blockrenderer) [draw, solid, minimum height = 10pt, fill=black!20, below left = -40pt and -220pt of result, align = center]{msg \\ sema\_wait};
\node (blockbrowser) [draw, solid, minimum height = 10pt, fill=black!20, below left = -80pt and -220pt of result, align = center]{msg \\ cv\_wait};

\node (result1) [minimum height = 10pt, below left = -80pt and -130pt of result, align = right] {renderer \\ waits browser \\to signal \\ semaphore};
...
\draw[->, thick] (blockbrowser.west) .. controls ([xshift=1cm]result1) .. pic{ext}(blockrenderer.west);
\draw[->, thick] (blockrenderer.east) .. controls ([xshift=-2cm]result2) .. pic{ext}(blockbrowser.east);
