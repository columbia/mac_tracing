...
def step_and_record(process, debugger, options):
	...
	while True:
		...
		try:
			...
			#if '_ReconfiguredProc' in top_frame:
			if '->  0x1043fac26' in asm_code and skip == 1:
				skip = 0
				...
				ret = run_commands(debugger, ['register write rip 0x1043fac28'])
				print "WLM end try to writ pc 0x1043fac28",  ret
			...
			if options.normal_endf in top_frame or options.spin_endf in top_frame:
				print 'Break after ', count, ' insts at ', top_frame
				print asm_code
				ret = run_commands(debugger, ['bt'])
				print ret
				break
