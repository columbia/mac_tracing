...
def step_and_record(process, debugger, options):
	...
	while True:
		...
		try:
			...
			# add to capture where objc_msgSend in ReconfiguredProc
			#if 'ReconfiguredProc' in top_frame and Begin_Reconfig == 0:
				#Begin_Reconfig = 1
			#if Begin_Reconfig == 1 and 'CGSPostLocalNotification' in top_frame:
				#Begin_Reconfig = -1
			...
			print asm_code
			if 'retq' in asm_code:
				ret = run_commands(debugger, ['register read rax'])
				print 'read rax:', ret
...
def skip_and_step(process, debugger, options):
	Symbol_prev = None
	cur_sym_step_in_count = 0
	count = 0
	while True:
		...
		try:
			top_frame, thread_info, asm_code = run_step_inst(debugger)
			if '->  0x10bc8393f' in asm_code:
				#print "WLM begin try to write pc"
				#ret = run_commands(debugger, ['register read'])
				ret = run_commands(debugger, ['register read rax'])
				print ret
				
				if 'activeDisplayNotificationHandler' in ret:
					print "WLM begin try to write pc"
					ret = run_commands(debugger, ['register read'])
					print ret
					#ret = run_commands(debugger, ['memory read/2xw `$rbp + 8`'])
					#print "original pc = ", ret
					ret = run_commands(debugger, ['register write rip 0x10bc83941'])
					print "WLM end try to writ pc 0x10bc83941",  ret
					ret = run_commands(debugger, ['register read rip'])
					print "writen rip ", ret
					ret = run_step_inst(debugger)
					print "ret"
					run_commands(debugger, ['br del 1'])	
...
##
	...
	parser.add_option('-b', '--breakpoint',
		...
		#default = 'CGSWaitThroughReconfigurationCycle')
	...
	process.Continue()
	skip_and_step(process, debugger, options)

	#options.breakpoint = 'notify_datagram_handler'
	#set_breakpoint(debugger, options)
