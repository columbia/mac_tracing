	...
	while True:
		...
		try:
			...
			print asm_code
					...
					#continue
			if 'display_notify_proc' not in top_frame:
				ret = run_thread_return(debugger)
				print 'thread finish at ' + top_frame
	...
	while True:
		...
		try:
			...
			if Begin_record == 1 and 'callq  *%rax' in asm_code:
				...
				print ret
				#if 'ReconfiguredProc' in ret or 'display_notify_proc' in ret:
			...
			if 'CGSPostLocalNotification' in top_frame and Begin_record == 0:
				Begin_record = 1
				print "Begin_record handlers"
				#ret  = run_commands(debugger, ['register read'])
				#print ret
				#obj_addr = get_value_from_expr(addr)
				#print "buffer pointer in :  ", hex(obj_addr + 0x48)
				#print "buf_size in : ", hex(obj_addr + 0x58)
				
				#ret = run_commands(debugger, ['po `*(int *)($rdi+0x58)`'])
				#print "direct read size expr = ", hex(int(ret,10))

				#ret = run_commands(debugger, ['memory read `*(int**)($rdi + 0x48)`'])
				#print "buffer = ", ret
				#break;

			if 'retq' in asm_code and 'CGSPostLocalNotification' in top_frame:
				Begin_record = 0
				print "End_record handlers"
