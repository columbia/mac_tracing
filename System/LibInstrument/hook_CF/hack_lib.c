...
bool prepare_detour(void * func, struct hack_handler *hack_handler_ptr)
{
...
int main(int argc, char * argv[])
{
	...
	struct hack_handler hack_handler_info;
	memset(&hack_handler_info, 0, sizeof(struct hack_handler));
	prepare_detour(CFRunLoopAddObserver, &hack_handler_info);
	void * sym_vm_addr = get_local_sym_in_vm(&hack_handler_info, sym);
	if(sym_vm_addr == NULL) {
		printf("Not found\n");
		exit(EXIT_FAILURE);
	...
	memset(&hack_handler_info, 0, sizeof(struct hack_handler));
	prepare_detour(CFRunLoopAddObserver, &hack_handler_info);
	sym_vm_addr = get_local_sym_in_vm(&hack_handler_info, "CFRunLoopAddObserver");
	if(sym_vm_addr == NULL) {
		printf("Not found\n");
		exit(EXIT_FAILURE);
	}
...
//
//	sym_vm_addr = get_local_sym_in_vm(&hack_handler_info, sym);
//	if(sym_vm_addr == NULL) {
//		exit(EXIT_FAILURE);
//	}
}
