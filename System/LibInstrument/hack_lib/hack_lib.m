/*for testing objective-C frameworks only*/
...
{
	char const *sym = "[NSApplication run]";
	struct hack_handler hack_handler_info;
	memset(&hack_handler_info, 0, sizeof(struct hack_handler));
	if (prepare_detour(NSApplicationMain, &hack_handler_info)) {
		void * sym_vm_addr = get_local_sym_in_vm(&hack_handler_info, sym);
		if(sym_vm_addr == NULL) {
			printf("Not found\n");
			exit(EXIT_FAILURE);
